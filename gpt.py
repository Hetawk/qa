...
def generate_answers_and_options(questions, output_file):
    if not questions:
        logging.error("No questions provided.")
        return

    data_dict = [{'id': i, 'question': q, 'query': q}
    ...
    for line in data_dict:
        ...
        if "error" in response_json and response_json["error"] == "quota_exceeded":
            logging.error("Quota exceeded during processing.")
            break
        background = ""
    ...
    answers = complete(prompts, data_dict=data_dict, max_tokens=300)
...
if __name__ == "__main__":
    # Provide your questions directly in this list
    questions = [
    ...
    output_file = 'ekd_prompt.json'
    generate_answers_and_options(questions, output_file)
